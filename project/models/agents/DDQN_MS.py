...
class DDQN_Agent(Agent):
    def __init__(self, env, args, model, device):
        ...
        super(DDQN_Agent,self).__init__(env, args, model, device)
        ###########################
        ...
        #for m step, try to have pull from agent runner
        self.m_step = 3
    ...
    def train(self, episode=None, step=None, current_tuple=None):
        ...
        with torch.no_grad():
            ...
            target_Q_A = torch.from_numpy(training_rewards).to(self.device) + (target_Q_A * self.gamma**self.m_step)
