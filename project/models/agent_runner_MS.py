...
class AgentRunner():
    def __init__(self, env, args, agent, model_class):
        ...
        #vehicles for multi_step 3, can set to 1, n should be ported to agent for loss function
        self.m_steps = 3  # set as variable n
        self.state_buffer = deque(maxlen=self.m_steps)
        self.reward_buffer = deque(maxlen=self.m_steps)
        self.action_buffer = deque(maxlen=self.m_steps)
    ...
    #Functions for m_step implimentation
    def to_mini_buffer(self, current_observation, action, reward):
        self.state_buffer.append(current_observation)
        self.action_buffer.append(action)
        self.reward_buffer.append(reward)

    def from_mini_buffer(self, next_observation, episode_complete):
        current_observation = self.state_buffer[0]
        action = self.action_buffer[0]
        reward = 0
        for s in range(len(self.reward_buffer)): reward += self.agent.gamma**s * self.reward_buffer[s]
    ...
    def train(self):
        ...
        for i in range(self.start_episode, self.num_episodes):
            ...
            while not episode_complete:
                ...
                self.to_mini_buffer(current_observation, action, reward)
                if ((not episode_complete) and (len(self.state_buffer) < self.m_steps)): continue

                current_tuple = self.from_mini_buffer(next_observation, episode_complete)
                ...
                if self.agent.can_train():
                   loss = self.agent.train(episode=i, step=current_step, current_tuple=current_tuple)  # is this right???
                   episode_loss += loss
